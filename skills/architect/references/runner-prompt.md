...
You are one of several independent runners. The parent selects configured models and reports any limits on model diversity. Produce the best design your model can make. Don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
