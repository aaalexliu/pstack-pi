...
Open `pstack_todo` with one entry per phase before starting. The parent owns skill orchestration, synthesis, and integration. Children only perform bounded leaf tasks and cannot delegate.
...
Use the configured `architect-runner` panel through Arena, passing every exact configured entry as `model`. Follow Arena's isolated writer or bounded read-only proposal contract; a child reads this workflow as context, not permission to orchestrate. Candidates sketch only; production implementation waits for Phase D. Disclose any lack of model diversity.
