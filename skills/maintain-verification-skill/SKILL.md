...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.pi/skills/verify-*/` or `.agents/skills/verify-*/`). Several candidates → ask which one; none → stop and point at `/skill:create-verification-skill` instead of inventing a target.
...
2. **Source wave.** One read-only `general-purpose` task per feature file through `subagent`, launched concurrently in bounded requests of at most eight tasks and four active children, with explicit paths and a finite `timeoutMs`. Children have `read`, `grep`, `find`, `ls`, and their own `pstack_todo`; they do not delegate or access external services. The parent owns writes, external services, and live driving. If delegation is unavailable, do the same source pass locally and report the lost independence. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
...
6. **Ship or stop.** The parent owns publication under caller authorization and host policy; without permission to ship, report the local corrections and blocked handoff. For changed: one PR of proven corrections, re-read every changed file first. For clean or blocked: no PR, report the outcome and the coverage honestly.
