...
5. Loop, one hypothesis per iteration:
   ...
   Each iteration ends in a check before the next begins (the **sequence-verifiable-units** principle skill). If the run is unattended and no wake tool exists, record durable checkpoints and a resume command instead of inventing a watcher. Autonomous Run is not bundled; do not invent its stop rule.
