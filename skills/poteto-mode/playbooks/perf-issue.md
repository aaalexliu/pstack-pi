...
1. Capture a baseline trace via available surface-control tools.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using `poteto-agent` and the configured role model for judgment. Review the diff. Capture a post-fix trace.
...
For sustained improvement against a metric rather than a one-off fix, use the Hillclimb playbook ([Hillclimb](hillclimb.md)).
