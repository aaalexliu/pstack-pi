...
4. Verify each component against its baseline via image diff on the matching surface via available surface-control tools. A nonzero diff is a fail. Investigate the pixel delta. Loop per component with an explicit local checkpoint until the diff is zero.
