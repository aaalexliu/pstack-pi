...
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`), not a similarly named workflow. That includes "babysit this", "get it green", "address the bugbot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling. The playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
...
Babysit details live in `playbooks/babysit.md` (mode declaration, forge resolve, merge-frontier discipline, anti-topology rules, and active-session forge checkpoints without inventing a watcher). Shipping details live in `playbooks/shipping.md` (independent per-PR verdicts, patch-id validity, contiguous landing from the root, and one-at-a-time merge). Bugbot triage uses `references/bugbot-triage.md`.
...
The parent opens `pstack_todo` whose first items are the matched playbook's steps, copied in verbatim, before any task-specific todos. A step you choose not to do stays in the list with a one-line `skip: <reason>`. Match the task to a playbook below. Open bundled files and copy their steps verbatim. For still-omitted workflows (Autopilot-full, Autopilot-stack), use the fallback rather than an absent file. Children return progress as text; their optional `pstack_todo` owns only their leaf checklist.
