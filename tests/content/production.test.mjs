...
test('production contains exactly the reviewed copied and adapted workflow set', async () => {
  ...
  assert.equal(inventory.byDestination.size, 104);
  ...
  assert.equal(manifest.files.filter((file) => file.kind === 'transform').length, 52);
  assert.equal(manifest.files.filter((file) => file.kind === 'replace').length, 0);
  assert.equal(manifest.files.filter((file) => file.kind === 'omit').length, 55);
