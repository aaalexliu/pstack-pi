...
test('cmux coalesces writes, uses owned keys, and clears after pending work', async () => {
  const { run, latest } = fixture();
  run.finish();
  /** @type {string[][]} */ const calls = [];
  let release = () => {};
  const gate = new Promise((resolve) => { release = () => resolve(undefined); });
  const sidebar = new Sidebar('workspace:test', async (args) => { calls.push(args); if (calls.length === 1) await gate; });
  ...
  assert.equal(calls[0][0], 'set-status');
  assert.equal(calls[1][0], 'clear-status');
